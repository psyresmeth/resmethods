...
- *Theory-driven* methods, which can include:
  ...
  - Including predictors that are captured in the data 
...
Results are reported in the same way you report simple linear regression. You only need to add a sentence reporting coefficient of each predictor. In addition, Tables can be particularly useful with multiple regression - especially with a lot of predictors. 
...
It looks like performance was best when there was no noise, with the worst performance when there was loud noise.
...
---
# Example of reporting a 1-way ANOVA

.large[
"A one-way between subject anova was conducted to examine the effects noise on performance. There was a significant main effect of noise conditions on performance, *F*(2, 147) = 134.56, *p* < .001, $\eta^2$ = .647. A follow-up post-hoc analysis using Tukey HSD shows that performance was best when there was no noise (*M*= 7.91, *SE*=.13) with the worst performance when there was loud (*M*= 4.94, *SE*=.13) and then quiet (*M*= 6.96, *SE*=.13) noise (all *ps* $\le$ .001)."
]
