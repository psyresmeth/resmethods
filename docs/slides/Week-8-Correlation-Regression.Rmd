...
The sign **(+ or -)** tells you the direction of the relationship. 

The number **(between 0 and 1)** tells you the strength of the relationship.

We calculate a coefficient between -1 and +1. The closer the value is to 1 (+or-), the stronger the relationship.

.center[
![:scale 90%](images/00/corr-direction.png)
...
What we want to see now is to what extent the values *vary together*. I.e. as one goes up, does the other? 
...
pairs.panels(crime_corr, stars = TRUE)
```
...
.pull-left[
You can use ‘apa.cor.table’ function from the apaTables package to create a nicely formatted APA table showing the means, standard deviation, and correlation with confidence intervals.


```{r eval = TRUE}
crime_corr2 <- select(crime, FoC, H, E, X, A, C, O)
```

```{r fig.height = 5, eval = FALSE}
apa.cor.table (crime_corr2, filename = "Correlation_Table.doc", table.number = 1, show.conf.interval = TRUE, landscape=TRUE)
```
The correlation table is created as a word document and saved in your working directory.

]

.pull-right[
![:scale 90%](images/00/apa-corr-table.png)
]
